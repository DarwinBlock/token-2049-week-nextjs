...
export default function EventModerator() {
	...
	const eventsToMod = useMemo(() => {
		if(events && events.length > 0){
			if(activeTable === 'all')
				return events;
			else if(activeTable === 'verified')
				return events.filter(event => event.verified === 1);
			else
				return events.filter(event => event.verified === 0);
		}
		else 
			return [];
	}, [events, activeTable]);
	...
	return (
		<div className="flex flex-col w-full font-primary items-center pt-[124px] mb-[4rem] px-32">
			...
			<div className="flex items-center my-[2rem] justify-between w-full">
				<div className="flex">
					<button onClick={() => setActiveTable('all')} className={"font-primary p-2 px-3 rounded-md text-sm font-semibold text-primary-500 " + (activeTable === 'all'? 'bg-primary-200':'bg-transparent')}>All Events</button>
					<button onClick={() => setActiveTable('unverified')} className={"font-primary p-2 px-3 rounded-md text-sm font-semibold text-primary-500 " + (activeTable === 'unverified'? 'bg-primary-200':'bg-transparent')}>Unverified</button>
					<button onClick={() => setActiveTable('verified')} className={"font-primary p-2 px-3 rounded-md text-sm font-semibold text-primary-500 " + (activeTable === 'verified'? 'bg-primary-200':'bg-transparent')}>Verified</button>
				</div>
			...
			<ModeratorTable 
				...
				events={eventsToMod} 
				headingsArr={['Event', 'Organiser', 'Image', 'Date', 'Timings', 'Registeration Link', 'Category', 'Ticket type', 'Price', 'CTA']}
			/>
